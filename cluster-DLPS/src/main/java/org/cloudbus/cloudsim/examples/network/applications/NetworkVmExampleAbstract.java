...
abstract class NetworkVmExampleAbstract {
    public static final int MAX_VMS_PER_HOST = 4;
...
/*        CloudletTask task = new CloudletExecutionTask(
        ...
        */
        addExecutionTaskLength(cloudlet, NETCLOUDLET_EXECUTION_TASK_LENGTH);
    }
    
    protected static void addExecutionTaskLength(NetworkCloudlet cloudlet, int length) {
        CloudletTask task = new CloudletExecutionTask(
            cloudlet.getTasks().size(), length);
        task.setMemory(NETCLOUDLET_RAM);
        cloudlet.addTask(task);
    }
